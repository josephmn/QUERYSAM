...
xAMPasarCAByDET_MOVI2

 
 -----------------------------------------------------------
 SP_HELPTEXT xAMPasarCAByDET_MOVI2
 
ALTER  proc  xAMPasarCAByDET_MOVI2        
as      
...
select nu_correla from BD_MOVISTARdbo.CAB_MOVI       
...
--IF @@ERROR <> 0  BEGIN GOTO ERROR END      
 ...
 Begin      
 UPDATE BD_MOVISTARdbo.CAB_MOVI   SET  ch_status = 'S' WHERE nu_correla   = @nu_correla      
...
nu_UniTotal,nu_UniXEmp,vc_Linea, vc_SubLinea, nu_Total,nu_TotIgv ,nu_TotDsct ,nu_Igv ,nu_Dscto ,ch_Bonifica, qfaccon1,ch_CodPromo ,ch_CodMotDe ,ch_fecVenci,  
 nu_Precio,ch_Lin, ch_UniCom ,nu_FacConve ,ch_kits, nu_PreDev ,id ,ch_NumItem ,ch_CodAlm, nu_Peso ,nu_Kgr ,ch_Tipo, ch_NomImpuesto ,nu_ValImpuesto FROM BD_MOVISTARdbo.DET_MOVI  WHERE nu_correla   =  @nu_correla   
...
ch_Status ,ch_ForPago ,ch_Retomado, nu_Total,nu_Dscto ,nu_Igv ,nu_PesTotal ,ch_NoPedido, ch_Calcular, vc_NomVende,dt_FecInici,dt_FecTermi ,ch_FecInici ,ch_FecTermi ,ch_Repaso,  
 ch_FueRuta ,ch_TipDoc, vc_Giro,vc_NroNextel, ch_CodMotDe, ch_CodCanal ,id  from BD_MOVISTARdbo.cab_MOVI  WHERE nu_correla   =  @nu_correla      
 ...
 select ch_nopedido,ch_status,* from cab_movi ORDER BY NU_CORRELA ASC 
 
 DELETE FROM CAB_MOVI WHERE NU_CORRELA BETWEEN  1 AND 28
 DELETE FROM DET_MOVI WHERE NU_CORRELA BETWEEN  1 AND 28
 
 select * from newsh where S4Future12  BETWEEN  1 AND 28
   
 
 
 
 select ch_nopedido,ch_status,* from cab_movi where nu_correla  = 14 
 
 select *  from det_movi where nu_correla  = 27
 ...
 SELECT *  FROM SCREEN WHERE NUMBER LIKE '%40421%'
 
 
 INSERT INTO SCREEN (MODULE,NAME,NUMBER,SCREENTYPE)
 VALUES ('OM','Import Tablas Pedidos Movist a Sol','4042200','S')
 
 
 select * from Access where ScrnNbr ='40422'

 select * from Access where ScrnNbr ='40422'


delete from Access where ScrnNbr ='40422'
---------------------------------------------------
SELECT  * FROM xAM_PedNexSolH WHERE   (NX_ch_HorInicioNum <=  GpoHoraMaxNum) and (SoAd_ShipToId = 'DEFAULT') and (rtrim(convert(char(30),NX_dt_FecInici,1)) = rtrim(convert(char(30),getdate(),1))) order by NX_ch_CodDistr,NX_nu_Correla  


SELECT * FROM xAM_PedNexSolH WHERE (NX_ch_HorInicioNum <=  GpoHoraMaxNum) and (SoAd_ShipToId = 'DEFAULT') and (rtrim(convert(char(30),NX_dt_FecInici,1)) = rtrim(convert(char(30),getdate(),1))) order by NX_ch_CodDistr,NX_nu_Correla  


 sp_helptext  xAM_PedNexSolH
...
--and   CONVERT(char,dbo.xAM_Nex_CAB_MOVI.nu_Correla)  NOT in (select NroPedidoNextel from xAM_PedNextelEnSolomon Where NroPedidoNextel <> ' )  
  
--and dbo.xAM_Nex_CAB_MOVI.ch_CodClien not in ('40668627')  
  
sp_helptext  xAM_Nex_CAB_MOVI

select  *   
from CAB_MOVI    
where ch_status = 'T' and ch_nopedido='  
...
select * from xAM_PedNexSolH


select * from cab_movi where 
'00019860817'