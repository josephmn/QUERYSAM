SELECT XVR_Sales_Report."ShipperID", XVR_Sales_Report."InvcNbr", XVR_Sales_Report."InvcDate", 
XVR_Sales_Report."ValorVta", XVR_Sales_Report."Afecto", XVR_Sales_Report."No_Afecto", XVR_Sales_Report."IGV",
 XVR_Sales_Report."Otros_tax", XVR_Sales_Report."All_Tax_Total", XVR_Sales_Report."TotInvc",
 XVR_Sales_Report."PerPost", XVR_Sales_Report."doctype", XVR_Sales_Report."TipoDoc", XVR_Sales_Report."cancelled",
 XVR_Sales_Report."FactRel", XVR_Sales_Report."TaxRate", GLSetup."Addr1", GLSetup."Addr2", GLSetup."CpnyName",
 GLSetup."LedgerID", Customer."Name", Customer."TaxLocId", vs_Company."Master_Fed_ID"
 FROM { oj (("AMCAF2015APP"."dbo"."XVR_Sales_Report" XVR_Sales_Report
 INNER JOIN "AMCAF2015APP"."dbo"."GLSetup" GLSetup ON XVR_Sales_Report."CpnyID" = GLSetup."CpnyId")
  INNER JOIN "AMCAF2015APP"."dbo"."vs_Company" vs_Company ON XVR_Sales_Report."CpnyID" = vs_Company."CpnyID")
  LEFT OUTER JOIN "AMCAF2015APP"."dbo"."Customer" Customer ON XVR_Sales_Report."CustID" = Customer."CustId"}
 WHERE (XVR_Sales_Report.perpost between '201511' AND '201511')

and  invcnbr  ='F770000005'
 ORDER BY XVR_Sales_Report."InvcDate" ASC


--******************************
sp_helptext XVR_Sales_Report
...
Select '',a.RefNbr, a.CustId, DocDate, SO.city, a.SlsPerid , SO.State ,           
 ...
 as ValorVta,           
 'Afecto' =           
 sum(case when a.TaxId00 = 'IGV1' then a.TxblTot00 when a.TaxId00 = 'IGV' then a.TxblTot00  when a.TaxId01 = 'IGV' then a.TxblTot01 when a.TaxId02 = 'IGV' then a.TxblTot02 when a.TaxId03 = 'IGV' then a.TxblTot03 Else 0 End)  -isnull((select isnull(tranAmt  ,0) from artran where refnbr = a.refnbr and acct ='401130'),0),          
        'No_Afecto' =          
...
select ShipperID,RefNbr,CustId,DocDate,city,SlsPerid,State ,      
(case when DocType='CM' then ValorVta *-1 else ValorVta end) ValorVta,      
(case when DocType='CM' then Afecto *-1 else Afecto end) Afecto,      
(case when DocType='CM' then No_Afecto *-1 else No_Afecto end) No_Afecto,      
(case when DocType='CM' then IGV *-1 else IGV end) IGV,      
(case when DocType='CM' then Otros_Tax *-1 else Otros_Tax end) Otros_Tax,      
(case when DocType='CM' then Total_Imp *-1 else Total_Imp end) Total_Imp,      
(case when DocType='CM' then OrigDocAmt *-1 else OrigDocAmt end) OrigDocAmt,      
PerClosed ,PerPost,Status,CpnyId ,      
BatNbr,CustOrdNbr,OrdNbr,DocType,SOTypeID,Behavior,TipoDoc,cancelled,FactRel,TaxRate  from    
(       
Select a.banksub as shipperid,a.RefNbr, a.CustId, DocDate, A.BATNBR city, a.SlsPerid , A.BANKID State,           
 sum(case when a.TaxId00 = 'IGV1' then a.TxblTot00 when a.TaxId01 = 'IGV' then a.TxblTot01 when a.TaxId02 = 'IGV' then a.TxblTot02 when a.TaxId03 = 'IGV' then a.TxblTot03 Else 0 End)          
 + Round((origdocamt-(Case when a.TaxId00 in ('IGV1') then a.TxblTot00 when a.TaxId01 in ('IGV') then a.TxblTot01     
 when a.TaxId02 in ('IGV') then a.TxblTot02 when a.TaxId03 in ('IGV') then a.TxblTot03 else 0 end ) -   (a.TaxTot00 + a.TaxTot01 + a.TaxTot02 +      a.TaxTot03)),2)   as ValorVta,           
 sum(case when a.TaxId00 = 'IGV1' then a.TxblTot00 when a.TaxId01 = 'IGV' then a.TxblTot01 when a.TaxId02 = 'IGV' then a.TxblTot02     
  when a.TaxId03 = 'IGV' then a.TxblTot03 Else 0 End)     + Round((origdocamt-(Case when a.TaxId00 in ('IGV1') then a.TxblTot00     
 when a.TaxId01 in ('IGV') then a.TxblTot01 when a.TaxId02 in ('IGV') then a.TxblTot02 when a.TaxId03 in ('IGV') then a.TxblTot03 else 0 end )       
 - (a.TaxTot00 + a.TaxTot01 + a.TaxTot02 +  a.TaxTot03)),2)    as Afecto,           
-- 'Afecto' =           
...
--then a.TxblTot03 else 0 end ) - (a.TaxTot00 + a.TaxTot01 + a.TaxTot02 + a.TaxTot03)),2),          
 ...
 (Case when a.TaxId00 = 'IGV1' then a.TaxTot00 when a.TaxId01 = 'IGV' then a.TaxTot01 when a.TaxId02 = 'IGV' then a.TaxTot02 when a.TaxId03 = 'IGV' then a.TaxTot03 Else 0 end),          
 'Otros_Tax' = sum(Case when a.TaxId00 <> 'IGV1' then a.TaxTot00 when a.TaxId01 <> 'IGV' then a.TaxTot01 when a.TaxId02 <> 'IGV' then a.TaxTot02 when a.TaxId03 <> 'IGV' then a.TaxTot03 Else 0 end),          
 sum(Case when a.TaxId00 = 'IGV1' then a.TaxTot00 when a.TaxId01 = 'IGV' then a.TaxTot01 when a.TaxId02 = 'IGV' then a.TaxTot02 when a.TaxId03 = 'IGV' then a.TaxTot03 Else 0 end) +     
 sum(Case when a.TaxId00 <> 'IGV1' then a.TaxTot00 when a.TaxId01 <> 'IGV'     
      
then a.TaxTot01 when a.TaxId02 <> 'IGV' then a.TaxTot02 when a.TaxId03 <> 'IGV' then a.TaxTot03 Else 0 end)          
 as Total_Imp,          
 a.OrigDocAmt, a.PerPost PerClosed, a.PerPost,''Status,a.CpnyId,BatNbr,''CustOrdNbr,''OrdNbr,a.DocType,''SOTypeID,''Behavior,a.User6 as TipoDoc, cancelled=0,          
 FactRel = Isnull(FR.InvcNbr ,''), a.user3 as TaxRate          
From ARDoc_CAF a          
LEFT JOIN PLCMInvcnbr_top1 FR on FR.CpnyID=a.CpnyID and FR.CustID=a.CustID and FR.DocType=a.DocType and FR.RefNbr=a.refnbr           
 --INNER JOIN SoAddress SO on SO.CustId = a.CustId and SO.ShipToID = a.user5          
