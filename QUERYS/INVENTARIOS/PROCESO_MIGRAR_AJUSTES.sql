...
set @idlocal = (select id from SGRPAM.SGRPAM2015.gen.local where identificador = @identificador )  
  ...
  select id,idAlmacenDestino,(select ltrim(rtrim(descripcion)) from SGRPAM.SGRPAM2015.GEN.TipoMovimiento where id =IdTipoMovimiento) TipoMovimiento  
  FROM SGRPAM.SGRPAM2015.INV.AjusteInventario M   
  where idAlmacenDestino IN  (@idlocal,'LIM01CEN000000000049')
...
set @CostoExtendido= (select sum(CostoExtendido) FROM SGRPAM.SGRPAM2015.INV.DetalleAjusteInventario where idajusteInventario = @idmnov and activo =1)  
set @sumCantidad= (select sum(Cantidad) FROM SGRPAM.SGRPAM2015.INV.DetalleAjusteInventario where idajusteInventario = @idmnov and activo =1)  
...
ReasonCd=(select ltrim(rtrim(abreviatura)) from SGRPAM.SGRPAM2015.inv.Ajustemotivo where id =IdMotivoAjuste),   
...
TranDesc=left(I.descr,30), TranType='AJ', UnitDesc=(select Abreviatura  from SGRPAM.SGRPAM2015.GEN.UnidadMedida where id = idUnidadMedida), UnitMultDiv='M',   
...
FROM   
SGRPAM.SGRPAM2015.INV.AjusteInventario M   
INNER JOIN SGRPAM.SGRPAM2015.INV.DetalleAjusteInventario D  ON M.Id = D.idajusteInventario  
