--------------*** ALTOMAYO ****
select        
rtrim(ltrim(ARBatNbr))+'ARA' Lote,'M' + right('000000'+ ltrim(ROW_NUMBER()OVER (PARTITION BY ARBatNbr  ORDER BY ARBatNbr)),6) Correlativo ,  convert(varchar, InvcDate, 103)FechaEmision, convert(varchar, DateVenc, 103) FechaVenc,  TipoDoc,      
...
dbo.FormatearDecimal(round((case  when(ShipperID = '' and doctype='CM')  or (ShipperID = 'CAFETERIA' and doctype='CM') then  Otros_Tax * -1 else   Otros_Tax end ),2),2, 0, 12)  OtrosTributos,Percepcion,      
...
----------******* CUSCO***********
select        
rtrim(ltrim(ARBatNbr))+'ARC' Lote,'M' + right('000000'+ ltrim(ROW_NUMBER()OVER (PARTITION BY ARBatNbr  ORDER BY ARBatNbr)),6) Correlativo ,  convert(varchar, InvcDate, 103)FechaEmision, convert(varchar, DateVenc, 103) FechaVenc,  TipoDoc,      
 '00'+ right( NroSerie,2) NroSerieS, NroSerie, Numero ,UltConsoli,      
(case when len(ltrim(X.custid ))  = 8 then  '1' when TipoDoc ='01' and  (len(ltrim(X.custid )) = 11 and (left(ltrim(X.custid ),2) ='10' or left(ltrim(X.custid ),2) = '20' or left(ltrim(X.custid ),2) ='15')) then '6'     
when X.custid  ='00000000000' then '0' else '1' end) TipoDocIdent,      
(case when X.custid  ='88888888888' then '-' when X.custid  ='00000000000' then '0' when TipoDoc ='03' and len(ltrim(X.custid ))  in (9,10,11) then right(left(rtrim(ltrim(X.custid )),10),8)       
