...
where BATNBR ='006701' AND ACCT IN ('121301','12110','131230') AND TRANAMT<>0  AND  TRANTYPE='CM'--AND refnbr IN ('B370019808','B370020017')
...
where BATNBR ='006701' AND ACCT   IN ('121301','12110','131230','149101') and rlsed =0 AND TRANAMT=0 --AND TRANTYPE='CM'--AND refnbr IN ('B370019808','B370020017')
 



SELECT  * FROM  ARDoc    where   BatNbr = '001097'  and RefNbr   = 'F030020937' 
SELECT  * FROM  ARDoc    where   BatNbr = '000968'  and RefNbr   = 'F030020886' 



SELECT  * FROM  ARTran     where   BatNbr = '001097'  and RefNbr   = 'F030020945' 
SELECT  * FROM  ARTran    where   BatNbr = '000968'  and RefNbr    = 'F030020888' 

select  BatNbr , tstamp ,  OrigDocAmt , *
---DELETE
 from  ARDoc    WHERE RefNbr  = 'F050029070'  AND DocType  = 'PA'  AND tstamp  = 0x00000001B54699AA 
select tstamp ,  *
---DELETE
 from  ARTran  WHERE RefNbr  = 'F050023669'   AND BatNbr  ='002665'   AND tstamp  = 0x00000001B5459D6E 

select  BatNbr ,  tstamp ,* from  ARDoc    WHERE RefNbr  = 'B050109289'  AND DocType  = 'PA'
select   * from  ARTran  WHERE RefNbr  = 'B050109289'   AND BatNbr  ='002665' 

select  tstamp , BatNbr ,Rlsed ,  PerPost ,RefNbr , Crtd_DateTime , Crtd_User , PerPost  , DocTypE ,DocBal , OrigDocAmt , CustId , *
 ----delete
 from ARDoc    where  RefNbr  ='F050029070' AND DocType ='PA' and BatNbr  ='001900' AND tstamp  = 0x00000001D218105A

 select  tstamp , BatNbr ,Rlsed ,  PerPost ,RefNbr , Crtd_DateTime , Crtd_User , PerPost  ,CustId , *
 ----delete
 from ARTran     where  RefNbr  ='F050029070' AND BatNbr  ='001900'   AND tstamp  = 0x00000001D2180AD2


 -- select  * INTO  ARTran20210102
 --from ARTran     where  RefNbr  ='F050029070' AND BatNbr  ='001900'  


--select   * into ARDoc20210102 from ARDoc    where  RefNbr  ='F050029070'


select doctype, ardoc.custid, t.custid,ardoc.refnbr, ardoc.origdocamt,ardoc.curyorigdocamt, t.CuryTranAmt,t.TranAmt,ardoc.DocBal,
ardoc.curyDocBal,ardoc.curyorigdocamt - t.CuryTranAmt as dif,   ardoc.origdocamt - t.TranAmt     as dif,OpenDoc,PerClosed
...
SELECT custid,batnbr,refnbr,
CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D' and trantype<>'CM' THEN  (curyTranAmt )*(-1) ELSE curyTranAmt  END)) as CuryTranAmt,
CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D'  and trantype<>'CM' THEN  (TranAmt )*(-1) ELSE TranAmt  END) )  as TranAmt
...
--FROM ARTRAN WHERE BATNBR='000238' and acct  not IN ('131230','121301','149101','141101') --and drcr ='C'
FROM ARTRAN WHERE BATNBR='001900' and acct IN ('121301')
group by custid,batnbr,refnbr
) t on ardoc.batnbr = t.batnbr and ardoc.refnbr = t.refnbr
WHERE ardoc.BATNBR='001900' ---- and t.refnbr in('351F906809','351F906812')
and ardoc.refnbr   in (
...
from ardoc inner join (
--SELECT batnbr,refnbr,CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D' and trantype<>'CM' THEN  (curyTranAmt + CuryTaxAmt00)*(-1) ELSE (curyTranAmt + CuryTaxAmt00) *-1 END)) as CuryTranAmt, CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D'  and trantype<>'CM' THEN  (TranAmt + CuryTaxAmt00)*(-1) ELSE (TranAmt + CuryTaxAmt00 )*-1 END) )  as TranAmt
SELECT custid,batnbr,refnbr,CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D' and trantype<>'CM' THEN  (curyTranAmt )*(-1) 
ELSE curyTranAmt  END)) as CuryTranAmt, CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D'  and trantype<>'CM' THEN  (TranAmt )*(-1) ELSE TranAmt  END) )  as TranAmt
--SELECT custid,batnbr,refnbr,CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D' and trantype<>'CM' THEN  (curyTranAmt )*(-1) ELSE curyTranAmt  END)) as CuryTranAmt, CONVERT(DEC(28,2),sum(CASE WHEN DrCr = 'D'  and trantype<>'CM' THEN  (TranAmt )*(-1) ELSE TranAmt  END) )  as TranAmt
--FROM ARTRAN WHERE BATNBR='000238' and acct  not IN ('131230','121301','149101','141101') --and drcr ='C'
FROM ARTRAN WHERE BATNBR='001900' and acct IN ('101000')
group by custid,batnbr,refnbr
) t on ardoc.batnbr = t.batnbr and ardoc.refnbr = t.refnbr
WHERE ardoc.BATNBR='001900' ---- and t.refnbr in('351F906809','351F906812')
and ardoc.refnbr   in (
...
select RefNbr ,  *
-- delete
 from ARDoc    where BatNbr = '004941'  and RefNbr = 'F050024583' --and  ApplBatSeq = '86' 

select  * from  ARTran      where BatNbr = '011269'  and RefNbr = 'F050020696' 

select RefNbr ,  * from ARDoc    where BatNbr = '011269'  and RefNbr = 'F050020696'  


select RefNbr , tstamp ,  *
-- delete
 from ARDoc    where BatNbr = '004941'  and RefNbr = 'F050024583'  and  ApplBatSeq = '66' 
...
select tstamp ,  *
--- delete
 from  ARTran      where BatNbr = '004941'  and RefNbr = 'F050024583'   and tstamp = 0x00000001BC2405F7
...
select   DISTINCT  A.CUSTID   , c.Name    from  ARDOC a
LEFT  JOIN   Customer   c  ON  A.CUSTID  =  c.CustId 
  where A.batnbr  = '007144' ---and refnbr  = 'B050084727'





select   DISTINCT  CUSTID      from  ARDOC  where   batnbr  = '007144'
...
select  rlsed,  * from  ARTRAN   where   batnbr  = '007144'   and  refnbr  = 'B050084727'  ---ND doctype ='PA'  


--select  * from  Gltran where  module  = 'AR' and  batnbr  = '007144' 

select  * from GLTRAN    where    batnbr  = '000968'   and   module  = 'AR' and  RefNbr = 'F110002928'



select  * from ARDOC    WHERE BATNBR  = '000968'    and  RefNbr = 'F110002928'

select  * 
--delete 
from  ARTran    WHERE BATNBR  = '000968'       and  DrCr = 'U'


select  * from ARDOC    WHERE BATNBR  = '000967'    and  RefNbr = 'F110002926'
select  * from  ARTran    WHERE BATNBR  = '000967'    and  RefNbr = 'F110002926'
...
----ERROR HUANCAYO
EXEC liberarAR '011185'

SELECT Module ,CrTot	,CtrlTot,	CuryCrTot,	CuryCtrlTot	,CuryDepositAmt ,DepositAmt, 
  rlsed,* FROM  Batch    WHERE BatNbr = '007144'  AND Module = 'AR'

--UPDATE  Batch SET  CrTot=3368.53	,CtrlTot = 3368.53,	CuryCrTot  = 3368.53,	CuryCtrlTot = 3368.53	,CuryDepositAmt = 3368.53 ,DepositAmt = 3368.53  WHERE BatNbr = '011185'  AND Module = 'AR'



select  TranAmt , CuryTranAmt , rlsed , * from ARTRAN where BatNbr = '007144' and RefNbr = 'F050018871' -- and RecordID = '517155' 
--select    * INTO ART001 from ARTRAN where BatNbr = '011185' and RefNbr = 'F050014342'  and RecordID = '517155' 
--delete from ARTRAN  where BatNbr = '011185' and RefNbr = 'F050014342'  and RecordID = '517155' 

select  OrigDocAmt , CuryOrigDocAmt , DocBal , CuryDocBal ,Status ,  * from ARDoc where BatNbr = '011185' and RefNbr = 'F050014342'  and BatSeq = '1' 
--select  * INTO ARDOC001 from ARDoc where BatNbr = '011185' and RefNbr = 'F050014342' -- and BatSeq = '1' 
 --delete from ARDoc  where BatNbr = '011185' and RefNbr = 'F050014342'  and BatSeq = '1' 

SELECT  Status,   * FROM Batch where BatNbr  = '006155' --- and Module = 'pa'


select refnbr,doctype,origdocamt,docbal,* from ardoc where refnbr  ='F050014342' AND DocType = 'PA'



select * from ardoc where BatNbr  ='002331' and RefNbr  ='F050009428'
select * from ARTran where BatNbr  ='002331' and RefNbr  ='F050009428'




--delete from ardoc where BatNbr  ='002331' and RefNbr  ='F050009428' and ApplBatSeq  = 65
--delete from ARTran where BatNbr  ='002331' and RefNbr  ='F050009428' and RecordID   = 109576




--delete from ardoc where BatNbr  ='001379' and RefNbr  ='F050008726' and BatSeq  = 65
--delete from ARTran where BatNbr  ='001379' and RefNbr  ='F050008726' and RecordID  = 109576
...
FROM ARDOC WHERE BATNBR='006155'
GROUP BY BATNBR
) D ON BATCH.BATNBR= D.BATNBR 
where BATCH.batnbr='006155'  and BATCH.MODULE='AR'
...
WHERE artran.BATNBR='001900' and acct ='121301' and t.refnbr ='B170000711'
