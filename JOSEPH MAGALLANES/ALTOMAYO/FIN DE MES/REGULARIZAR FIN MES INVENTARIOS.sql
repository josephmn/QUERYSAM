...
sum((case when vr_10630.TranType in ('RC', 'AJ', 'AC', 'AS', 'TR','IN') then vr_10630.QTY * vr_10630.Unitprice * vr_10630.InvtMult else 0 end  )) valor2   
	...
	INNER JOIN "AM2018APP"."dbo"."InventoryADG" InventoryADG ON Inventory."InvtID" = InventoryADG."InvtID"} WHERE (vr_10630.InvtID like '20%')
	and vr_10630.PerPost  = '201808' --AND vr_10630."BatNbr" ='004314'
...
from gltran G where acct IN  ('201101') and perpost  ='201808' and G.posted ='P'  --and batnbr ='004314'-- and trandesc  ='CARNE MOLIDA'  
...
sum((case when vr_10630.TranType in ('RC', 'AJ', 'AC', 'AS', 'TR','IN') then vr_10630.QTY * vr_10630.Unitprice * vr_10630.InvtMult else 0 end  )) valor2   
	...
	and vr_10630.PerPost   <= '201804'-- AND RefNbr  ='00000060'  and trandesc  ='CARNE MOLIDA'   
	group by vr_10630."BatNbr", vr_10630."RefNbr",vr_10630."InvtID",vr_10630."PerPost",vr_10630."TranType"
union all	
select 'GL',perpost,batnbr,REFNBR,acct,trantype,ROUND( sum(DRAmt)- sum(CrAmt),2) ,ROUND( sum(curyDRAmt)- sum(curyCrAmt)  ,2)
from gltran G where acct IN  ('211101') and perpost  <='201804' and G.posted ='P' --and RefNbr   ='00000060' and batnbr ='008716' and trandesc  ='CARNE MOLIDA'  
--and  module ='IN' --and batnbr ='003812'  --and
group by perpost,acct,batnbr,REFNBR,trantype
...
inner join SOShipLine D on H.ShipperID  = D.ShipperID where H.Cancelled   = 0 AND h.PerPost  ='201805' AND INBatNbr  ='004314'
group by  INBatNbr ,InvcNbr,D.InvtId
)y

inner join (
SELECT BatNbr ,RefNbr  ,InvtID ,SUM(QTY)CANTIDAD FROM INTRAN WHERE BatNbr   ='004314' AND RefNbr NOT IN (SELECT REFNBR FROM gltran WHERE MODULE  ='in' and perpost ='201805' )
and TranType  <>'CG'
...
SELECT  'IN'Module,vr_10630."perpost",vr_10630.BatNbr ,vr_10630.RefNbr,vr_10630."InvtID",vr_10630."trantype",
	...
	INNER JOIN "AMCAF2018APP"."dbo"."InventoryADG" InventoryADG ON Inventory."InvtID" = InventoryADG."InvtID"} WHERE (vr_10630.invtacct = '251101')
	and vr_10630.PerPost  <= '201805' --AND RefNbr  ='AI00000028' AND vr_10630.BATNBR  ='008482' 
	group by vr_10630."BatNbr", vr_10630."RefNbr",vr_10630."InvtID",vr_10630."PerPost",vr_10630."TranType"
	union all	
select 'GL',perpost,batnbr,REFNBR,acct,trantype,ROUND( sum(DRAmt)- sum(CrAmt),2) ,ROUND( sum(curyDRAmt)- sum(curyCrAmt)  ,2)
from gltran where acct like '251101%' and perpost  <='201805'-- and RefNbr   ='AI00000028' AND GLTran .BatNbr  ='008482'
--and  module ='IN' --and batnbr ='003812'  --and
group by perpost,acct,batnbr,REFNBR,trantype
