...
set @idlocal = (select id from SGRPAM.SGRPAM2018.gen.local where identificador = @identificador )        
  ...
  select id,idAlmacenDestino,(select ltrim(rtrim(descripcion)) from SGRPAM.SGRPAM2018.GEN.TipoMovimiento where id =IdTipoMovimiento) TipoMovimiento        
  FROM SGRPAM.SGRPAM2018.INV.AjusteInventario M         
  where idAlmacenDestino IN  ('LIM01CEN000000000049','LIM01CEN000000000048','LIM01CEN000000000047',@idlocal)      
  ...
  where     
   fechanegocio = '2016-07-18'  and m.activo =1        
      */    
...
set @CostoExtendido= (select sum(CostoExtendido) FROM SGRPAM.SGRPAM2018.INV.DetalleAjusteInventario where idajusteInventario = @idmnov and activo =1)        
set @sumCantidad= (select sum(Cantidad) FROM SGRPAM.SGRPAM2018.INV.DetalleAjusteInventario where idajusteInventario = @idmnov and activo =1)        
set @identificador = (select identificador from SGRPAM.SGRPAM2018.gen.local where id = @idAlmacenDestino )        
...
FROM         
SGRPAM.SGRPAM2018.INV.AjusteInventario M         
where id =@idmnov        
...
ReasonCd=(select ltrim(rtrim(abreviatura)) from SGRPAM.SGRPAM2018.inv.Ajustemotivo where id =IdMotivoAjuste),         
...
TranDesc=left(I.descr,30), TranType='AJ', UnitDesc=(select Abreviatura  from SGRPAM.SGRPAM2018.GEN.UnidadMedida where id = idUnidadMedida), UnitMultDiv='M',    
...
FROM         
SGRPAM.SGRPAM2018.INV.AjusteInventario M         
INNER JOIN SGRPAM.SGRPAM2018.INV.DetalleAjusteInventario D  ON M.Id = D.idajusteInventario        
inner join SGRPAM.SGRPAM2018.GEN.Producto P on D.idproducto = P.id          
