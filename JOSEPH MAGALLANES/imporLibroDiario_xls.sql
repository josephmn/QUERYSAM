...
--CREATE view imporLibroDiario_xls    
--as  

--CREATE  view [dbo].[imporLibroDiario_xls]       
--as        
...
CAST(FechaContable AS CHAR(10))FechaContable, CAST(FechaVence AS CHAR(10))FechaVence, CAST(FechaEmision AS CHAR(10))FechaEmision, CAST(Descr AS CHAR(30))Descr, CAST(Campo17 AS CHAR(1))Campo17, CAST(Codigo AS CHAR(25)) Codigo, CAST(dbo.FormatearDecimal(CrAmt,2, 0, 12)AS CHAR(20)) DrAmt, 
CAST(dbo.FormatearDecimal(DrAmt,2, 0, 12)AS CHAR(20)) CrAmt,dbo.FormatearDecimal(Start,2, 0, 12)Start    
from (    
...
isnull((case when module ='AR' then '0'+(right(rtrim(refnbr),6) )    
   when module ='AP' then (select top 1 (case when user6 in('01','02','03','07','08')then right(rtrim(ltrim(invcnbr)),6) else invcnbr end)  from apdoc where batnbr =t.batnbr and refnbr = t.refnbr and user6<>'' )   else '' end ),'') Numero,    
...
(case when module in('AR') then '140100&'+t.PerPost+'00&'+rtrim(t.BatNbr)+CAST( t.module+'A' AS CHAR(3))     
      when module in('AP') then '080100&'+t.PerPost+'00&'+rtrim(t.BatNbr)+CAST( t.module+'A' AS CHAR(3)) else '' end ) Codigo,    
v.Start          
...
WHERE BatNbr = '000372' AND Module = 'APA'
